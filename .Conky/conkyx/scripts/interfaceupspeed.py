#!/usr/bin/env python3
'''
This script uses the psutil library and time module to calculate the upload
speed of a specified network interface in bits per second (b/s), kilobits per
second (Kb/s), megabits per second (Mb/s), or gigabits per second (Gb/s).
It also formats the output to display the speed in an appropriate unit using the
...
def get_interface_speed(interface):
    # Get the current number of bytes sent
    current_bytes = psutil.net_io_counters(pernic=True)[interface].bytes_sent

    # Wait for one second
    time.sleep(1)

    # Get the number of bytes sent after one second
    next_bytes = psutil.net_io_counters(pernic=True)[interface].bytes_sent

    # Calculate the difference to get the upload speed
